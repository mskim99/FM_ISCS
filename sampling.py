...
def _unwrap_sampling_update(out, name="update_fn"):
    if not isinstance(out, tuple):
        raise TypeError(f"{name} must return a tuple, got {type(out)}")
    if len(out) < 2:
        raise ValueError(f"{name} must return at least 2 values, got {len(out)}")
    return out[0], out[1]
    ...
    def pc_sampler(model):
        ...
        with torch.no_grad():
            ...
            for i in range(sde.N):
                ...
                x, x_mean = _unwrap_sampling_update(
                    corrector_update_fn(x, vec_t, model=model),
                    name="corrector_update_fn",
                )
                time_corrector_tot += time.time() - tic_corrector
                tic_predictor = time.time()
                # x, x_mean = predictor_update_fn(x, vec_t, model=model)
                x, x_mean = _unwrap_sampling_update(
                    predictor_update_fn(x, vec_t, model=model),
                    name="predictor_update_fn",
                )
