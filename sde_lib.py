...
class VESDE(SDE):
    ...
    def marginal_prob(self, x, t):
        ...
        # print('self.sigma_min,self.sigma_max,t',self.sigma_min,self.sigma_max,t)
        mean = x
        return mean, std
    ...
    def prior_sampling_t(self, x, t):
        timestep = (t * (self.N - 1) / self.T).long()
        noise = torch.randn(x.shape) * self.discrete_sigmas[timestep]
    ...
    def discretize(self, x, t):
        ...
        adjacent_sigma = torch.where(
            timestep == 0, torch.zeros_like(t), self.discrete_sigmas.to(t.device)[timestep - 1].to(t.device)
        )
