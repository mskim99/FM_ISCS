...
def _call_module_maybe_temb(module, x, temb, m_idx=None):
    sig = inspect.signature(module.forward)   # bound method라 self는 제외됨
    n_params = len(sig.parameters)

    # Resnet block류: forward(x, temb)
    if n_params >= 2:
        return module(x, temb)

    print(
        f"[DEBUG] module at m_idx={m_idx} does not take temb: "
        f"{module.__class__.__name__}, input_shape={tuple(x.shape)}",
        flush=True,
    )

    # Attn / Upsample / Downsample / norm-like block류: forward(x)
    return module(x)
    ...
    def forward(self, x, time_cond):
        ...
        for i_level in range(self.num_resolutions):
            ...
            for i_block in range(self.num_res_blocks):
                ...
                # h = _call_module_maybe_temb(modules[m_idx], hs[-1], temb, m_idx=m_idx)
                m_idx += 1
        ...
        # h = _call_module_maybe_temb(modules[m_idx], h, temb, m_idx=m_idx)
        m_idx += 1
        h = modules[m_idx](h)
        m_idx += 1
        h = modules[m_idx](h, temb)
        ...
        for i_level in reversed(range(self.num_resolutions)):
            for i_block in range(self.num_res_blocks + 1):
                ...
                # h = _call_module_maybe_temb(modules[m_idx], torch.cat([h, tmp], dim=1), temb, m_idx=m_idx)
                m_idx += 1
