        ...
        if config.data.image_size == 28:
            self.res4 = nn.ModuleList(
                [
                    ...
                    ResidualBlock(
                        2 * self.nf, 2 * self.nf, resample=None, act=act, normalization=self.norm, dilation=4
                    ),
                ]
            )
        else:
...
class NCSN(nn.Module):
    def __init__(self, config):
        ...
        self.res1 = nn.ModuleList(
            [
                ConditionalResidualBlock(
                    self.nf, self.nf, config.model.num_scales, resample=None, act=act, normalization=self.norm
                ),
                ConditionalResidualBlock(
                    self.nf, self.nf, config.model.num_scales, resample=None, act=act, normalization=self.norm
                ),
        ...
        self.res2 = nn.ModuleList(
            [
                ConditionalResidualBlock(
                    self.nf, 2 * self.nf, config.model.num_scales, resample="down", act=act, normalization=self.norm
                ),
                ConditionalResidualBlock(
                    2 * self.nf, 2 * self.nf, config.model.num_scales, resample=None, act=act, normalization=self.norm
                ),
            ]
        )
...
class NCSNv2_256(nn.Module):
    ...
    def forward(self, x, y):
        ...
        print(x.shape)
        print(*([1] * len(x.shape[1:])))
        print(self.sigmas[y.long()])
        used_sigmas = self.sigmas[y.long()][0].view(x.shape[0], *([1] * len(x.shape[1:])))
